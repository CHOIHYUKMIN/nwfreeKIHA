...
**현재 버전**: v2.6 (2025-10-27 18:00:00)
...
- 검진 항목 기록 및 결과 관리
- 검진 일정 관리 및 대시보드
- **P2P 데이터 전송 (WebRTC)** ⭐ NEW (v2.6) - 인터넷 없이 기기 간 직접 전송
- **엑셀 데이터 내보내기/가져오기** (v2.4)
- **공개 URL 터널 지원** (v2.4)
- **실제 서버 상태 기반 온라인/오프라인 표시** (v2.5)
- **정확한 동기화 완료 시점 표시** (v2.5)
...
### 최근 주요 변경사항 (v1.0 → v2.6)

#### v2.6 (2025-10-27 18:00) - 🚀 P2P 데이터 전송 (WebRTC)
- ✅ **WebRTC P2P 데이터 전송**: 인터넷 없이 기기 간 직접 데이터 전송
- ✅ **QR 코드 기반 연결**: QR 스캔으로 간편하게 Offer/Answer 교환
- ✅ **오프라인 완전 지원**: WiFi나 인터넷 없이도 사용 가능
- ✅ **대용량 데이터 전송**: 청크 단위 전송으로 안정적인 대용량 데이터 전송
- ✅ **진행률 표시**: 실시간 전송 진행률 및 상태 표시
- ✅ **자동 데이터 병합**: 중복 ID 자동 처리로 안전한 데이터 병합
- ✅ **설정 화면 통합**: 설정 화면에서 데이터 보내기/받기 버튼 제공
- ✅ **양방향 통신**: 송신자/수신자 역할 자동 전환
- ✅ **에러 처리**: 연결 실패, 타임아웃 등 다양한 에러 상황 처리

**기술 스택**:
- WebRTC DataChannel (P2P 통신)
- QRCode.js (QR 생성)
- Html5-qrcode (QR 스캔)
- STUN 서버 (Google 공용)

**사용 방법**:
1. **송신자**: 설정 → P2P 데이터 전송 → "데이터 보내기" → QR 생성
2. **수신자**: "데이터 받기" → Offer QR 스캔 → Answer QR 생성
3. **송신자**: Answer QR 스캔 → 연결 완료 → 자동 전송
4. **수신자**: 데이터 수신 완료 → IndexedDB 자동 저장
...
- **최종 업데이트**: 2025-10-27 18:00

### 주요 설정 위치
- **P2P 데이터 전송**: 설정 화면 → P2P 데이터 전송 (오프라인) ⭐ NEW
- **자동 동기화**: 설정 화면 → 자동 동기화 토글
- **엑셀 내보내기/가져오기**: 설정 화면 → 엑셀 데이터 관리 섹션
- **공개 URL**: 설정 화면 → 서버 정보 섹션
- **버전 정보**: 헤더 우측 상단 (v2.6 / 2025-10-27 18:00:00)
...
---
*최종 업데이트: 2025-10-27 18:00 (v2.6 - P2P 데이터 전송)*
*문서 버전: 2.6*